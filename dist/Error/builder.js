 ...
 */
export function $MissingParameterError(emitter, target, value) {
 ...
 */
export function $UnexpectedTypeError(emitter, target, unexpected, expected) {
 ...
 */
export function $MismatchArrayLengthError(emitter, target, targetLength, expectedTargetLength) {
 ...
 */
export function $IndexOutOfBoundsError(emitter, target, receivedIndex, minMaxIndex) {
    throw new IndexOutOfBoundsError({
        ...
        Guide: `Ensure to only provide a index within the @MINIMUM = ${minMaxIndex[0]} and @MAXIMUM = ${minMaxIndex[1]} index available from parameter @${target} at ${emitter}()`
    });
}
/**
 * Build and emits the contents of `@UnknownHTMLTagError`.
 */
export function $UnknownHTMLTagError(emitter, target, unknownTag) {
